...
- 已存在于 AGENTS.md / architecture.md / roadmap.md 的内容，不要在计划或最终答复中重新复述。
- 每个任务先 search 定位，再读取必要文件范围；禁止默认全仓扫描。
- 单次任务不要重复读取同一文件，除非文件已被修改且必须验证。
- 不要为了“理解项目”读取无关页面、组件和历史文件。
- 测试优先 targeted test；最终阶段才运行全量验证。
- 测试输出只保留失败相关部分，不把大段成功日志重新分析。
- 不要主动创建 subagent。
- 不要主动进行第二轮代码审查，除非测试失败或任务明确要求。
- 达到 acceptance criteria 后立即结束。
- 最终回复控制在 30 行以内。